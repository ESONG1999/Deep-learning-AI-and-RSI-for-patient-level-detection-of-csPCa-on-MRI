...
class DenseNet_plus_1(nn.Module):
    """
        ...
        self.class_layers = nn.Sequential(
            OrderedDict(
                [
                    ...
                    ("out", nn.Linear(in_channels, 5)),
                ]
            )
        )

        self.out_layers = nn.Linear(5 + 1, out_channels)
    ...
    def forward(self, x: torch.Tensor, rsi_max: torch.Tensor) -> torch.Tensor:
        x = self.features(x)
        x = self.class_layers(x)
        rsi_max = rsi_max.to(x.dtype)
        x = torch.cat((x, rsi_max), 1)
        x = self.out_layers(x)
        return x
...
class DenseNet121_plus_1(DenseNet_plus_1):
